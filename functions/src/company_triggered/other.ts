...
import { incrementReadWriteCounts, logError, restrictedNumbersList } from "..";
...
// export const c_selfBasicsAndUpdateNetworkDocs_export = functions.region('asia-south1').runWith({ maxInstances: 100, timeoutSeconds: 540, memory: '1GB' }).firestore.document(`${companiesCollectionStringV2}/{cId}`).onWrite(async (change, context) => {
...
//             if (change.before.data()!.name != null && change.before.data()!.name != undefined)
//                 beforeName = change.before.data()!.name

//             if (change.before.data()!.actualCity != null && change.before.data()!.actualCity != undefined)
//                 beforeCity = change.before.data()!.actualCity

//             if (change.before.data()!.category != null && change.before.data()!.category != undefined)
//                 beforeCategory = change.before.data()!.category

//             if (change.before.data()!.testTrigger != null && change.before.data()!.testTrigger != undefined)
//                 beforeTestTrigger = change.before.data()!.testTrigger

//         }

//         if (change.after.exists && change.after.data() != null && change.after.data() != undefined) {
//             if (change.after.data()!.name != null && change.after.data()!.name != undefined)
//                 afterName = change.after.data()!.name

//             if (change.after.data()!.actualCity != null && change.after.data()!.actualCity != undefined)
//                 afterCity = change.after.data()!.actualCity

//             if (change.after.data()!.category != null && change.after.data()!.category != undefined)
//                 afterCategory = change.after.data()!.category

//             if (change.after.data()!.testTrigger != null && change.after.data()!.testTrigger != undefined)
//                 afterTestTrigger = change.after.data()!.testTrigger
//         }
...
//             // do nothing 
//             // console.log(`same so doing nothing`)
//         } else if (beforeCategory == 'grain' && afterCategory != 'grain' && (beforeName != '' || beforeCity != '')) {
//             try {
...
//                     writesCounter++
//                 }
//                 // const beforeTwoLetters: string = getOnlyLettersLowerCase(beforeName).substring(0, 2)
//                 // await db.collection('companyNames').doc(beforeTwoLetters).update({ 'companyNames': admin.firestore.FieldValue.arrayRemove(`${beforeName}, ${beforeCity}`) })
//                 // console.log(`removing ${beforeName} from ${beforeTwoLetters}`)
...
//                         writesCounter++
//                         // await logError(`NOTERR; formattedWithSpaces: ${formattedWithSpaces}, twoLettersAgain: ${twoLettersAgain}`)
//                     } else {
//                         // await logError(`NOTERR; formattedWithSpaces: ${formattedWithSpaces} did not add name split`)
//                     }
...
//             newNameArray = await getNameArray(afterData.name || '', afterData.keywords || [], afterData.products || [], afterData.productsV2 || [], afterData.category || '', categorySpecificKeywords || [])
